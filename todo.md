...
- [x] **Missing Zod validation**: `ipcMain.handle('settings:setBrowserMode')` casts `mode as any` instead of using Zod schema.
...
- [x] **Hidden stall warnings**: Stall nudge messages from `stall-detector.ts` are logged but not shown to the user in the UI.
- [ ] **Workflow Editor**: Add drag-and-drop step reordering support for better UX.
- [x] **Workflow Architecture**: Migrated step model from `act/observe/extract` to intent-based `navigate/do/collect/check` with jump-based branching, per-step `onFailure` policy, and updated editor UI.

## 🟢 Low Priority / Cleanup (Tech Debt)
- [x] **Dead Code (Logs & Context)**: `conversation-manager.ts` and `execution-logger.ts` are fully implemented but never imported. Wire them into `agent-executor.ts` to prevent context window limits and enable debugging logs.
- [x] **Dead Code (Orchestration)**: `task-planner.ts`, `advanced-task-executor.ts`, and `complex-task-executor.ts` are dead paths. Consolidate them and wire them into `browser:startAgent` or remove them.
- [x] **Unused Error Classes**: Remove or use `BrowserNotReadyError` and `StagehandConnectionError` in `errors.ts`.
...
- [x] **Hardcoded New Tab URL**: `newTab()` in `browser-manager.ts` hardcodes `https://google.com`.
- [x] **LLM Provider Support**: `task-evaluator.ts` only supports OpenAI, Anthropic, and Gemini. Add support for the remaining providers.
