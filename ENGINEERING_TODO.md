...
- [x] Cmd+Q does not quit the app cleanly on macOS; route the shortcut explicitly from both windows and bound asynchronous shutdown cleanup so a stuck browser close cannot hold the app open.
...
- [x] Make task cancellation, retry backoff, waits, and browser readiness abort-aware.
...
- [x] Add resume/recover APIs and UI actions for interrupted agent/workflow runs.
